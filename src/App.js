...
function App() {
  ...
  const generateColoringPage = async (inputPrompt) => {
    ...
    // Build the enhanced prompt for coloring page style
    const enhancedPrompt = `Children's coloring book page, black and white line art, simple bold outlines, ${searchPrompt}, no shading, no gray tones, no filled areas, clean lines for kids to color, white background, cute cartoon style, coloring page`;

    // Pollinations.ai endpoint - completely free, no API key needed!
    const imageUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(enhancedPrompt)}?width=800&height=800&seed=${Date.now()}&nologo=true`;
    ...
    try {
      ...
      // Add to recent pages
      setRecentPages(prev => {
        const newPage = { prompt: searchPrompt, url: imageUrl };
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        {isGenerating && (
          <div className="bg-white rounded-3xl shadow-xl p-12 mb-8 text-center border border-gray-100">
            ...
            <p className="text-gray-500">Making something magical for "{currentPrompt}"</p>
